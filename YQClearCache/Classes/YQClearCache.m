...
+ (long long)cacheSizeForUnit:(NSString *)unit {
    ...
    // 将文件夹大小转换为 M/KB/B
    long long totalSizeUnit = totalSize;
    if ([unit isEqualToString:@"M"]) {
        totalSizeUnit = totalSize / 1000.0f /1000.0f;
    } else if ([unit isEqualToString:@"KB"]) {
        totalSizeUnit = totalSize / 1000.0f;
    } else if ([unit isEqualToString:@"B"]) {
        totalSizeUnit = totalSize / 1.0f;
    } else {
        NSAssert(NO, @"单位错误，请给正确的单位");
    }
